...
const getBestNews = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(`
      SELECT title, introText, text, imagenUrl, categoryId, score, publishDate FROM news ORDER BY score DESC LIMIT 3
    `);
...
const getWorstNews = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(`
      SELECT title, introText, text, imagenUrl, categoryId, score, publishDate FROM news ORDER BY score ASC LIMIT 3
    `);
